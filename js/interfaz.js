...
function prepareInterfaz() {
    ...
    TweenMax.to($('#infoNombre'), 0, {
        marginTop: -500
    });
    TweenMax.to($('#buscar'), 0, {
        marginTop: -500
    });
    TweenMax.to($('#infoPulsa'), 0, {
        marginTop: window.innerHeight + 500
    });
    TweenMax.to($('#infoPulsa #animaPulsa'), 0, {
        backgroundPosition: "0 0"
    });

    TweenMax.to($('#buscador'), 0, {
        width: '0px'
    });
    TweenMax.to($('#buscar #cerrar'), 0, {
        width: '0px'
    });
...
function abrirBuscador() {
    ...
    TweenMax.to($('#buscador'), 1, {
        width: '200px',
        paddingLeft: '35px',
        ease: Cubic.easeOut
    });
    $('#buscador').css('cursor', 'text');
    TweenMax.to($('#buscar #cerrar'), 1, {
        display: "block",
        width: '242px',
        ease: Cubic.easeOut
    });
...
function meterInfo(nombre, departamento, empresa, foto, pulso) {
    esconderTodo();
    meterInfoTimeout = setTimeout(function() {
        entrarInfo(nombre, departamento, empresa, foto, pulso)
    }, 600);
}
...
function entrarInfo(nombre, departamento, empresa, foto, pulso) {
    ...
    $('#infoNombre #photoPersonal').css("background-image", "url('" + foto + "')");
...
function muestraPulso(pulso, delay) {
    TweenMax.to($('#infoPulsa'), 1, {
        marginTop: posInfoPulsa,
        ease: Cubic.easeOut,
        display: 'block',
        delay: delay
    });
    TweenMax.to($('#infoPulsa #animaPulsa'), 60 / parseInt(pulso), {
