...
function completeCorazon(collada) {
    var matCorazon = new THREE.MeshBasicMaterial({
        map: THREE.ImageUtils.loadTexture(RUTA_CORAZON_GRANDE),
        transparent: true
    }); // new THREE.MeshLambertMaterial( { selfillumination:true, map: THREE.ImageUtils.loadTexture(RUTA_CORAZON_GRANDE), transparent:true, color: 0xb00000, wireframe: false } );
    dae = collada.scene;
    dae.updateMatrix();
...
function createDust() {
    ...
    for (i = 0; i < parameters.length; i++) {
        ...
        dustMaterials[i] = new THREE.PointCloudMaterial({
            size: size,
            map: spr,
            blending: THREE.AdditiveBlending,
            depthTest: false,
